...
static int nn_(LeakyReLU_updateOutput)(lua_State *L)
{
  ...
  real slope = luaT_getfieldchecknumber(L, 1, "slope");
  ...
  if (inPlace) {
    TH_TENSOR_APPLY(real, input,                   \
                    if (*input_data <= 0) { \
                      *input_data = *input_data * slope;           \
                    });
    THTensor_(set)(output, input);
  } else {
    THTensor_(resizeAs)(output, input);
    TH_TENSOR_APPLY2(real, output, real, input,                         \
                     *output_data = (*input_data > 0) ? *input_data : *input_data*slope;);
...
static int nn_(LeakyReLU_updateGradInput)(lua_State *L)
{
  ...
  if (inPlace) {
    TH_TENSOR_APPLY2(real, gradOutput, real, input,    \
                     if ((*input_data) <= 0) { \
                       *gradOutput_data = *gradOutput_data * slope;           \
                         });
    ...
    TH_TENSOR_APPLY3(real, gradInput, real, gradOutput, real, input,    \
                     ...
                     else *gradInput_data = *gradOutput_data * slope;);                        \
  }
